...
context(`Tabs`, () => {
	...
	context(`Simple Tabs`, () => {
		...
		it(`Should select correct panel when tab clicked`, () => {
			...
			tabSequence.forEach((index) => {
				...
				newTabNumber = index;
				cy.addCustomEventListener(EVENTS.OUT.SELECTED_TAB_CHANGED, getExpectedDetailObj(TABS_ID, oldTabNumber, newTabNumber))
					.get('@tabsTabs')
					.eq(newTabNumber - 1)
					.click();
		...
		describe(`Keyboard interactions`, () => {
			...
			it(`Should select previous tab when LEFT pressed`, () => {
				...
				cy.addCustomEventListener(EVENTS.OUT.SELECTED_TAB_CHANGED, getExpectedDetailObj(TABS_ID, 2, newTabNumber))
					.get('@tabsButton2')
					.focus()
					.type('{leftarrow}');
				checkTabSelected(newTabNumber);
		...
		describe(`Observed attributes`, () => {
			it(`Should set correct tab when observed attribute changed`, () => {
				const tabNumber = 3;
				cy.addCustomEventListener(EVENTS.OUT.SELECTED_TAB_CHANGED, getExpectedDetailObj(TABS_ID, 1, tabNumber))
					.get('@tabs')
	...
	context(`Infinite Tabs with initially set selected tab`, () => {
		...
		describe(`Observed attributes`, () => {
			it(`Should select correct tab when observed attribute changed`, () => {
				const tabNumber = 3;
				cy.addCustomEventListener(EVENTS.OUT.SELECTED_TAB_CHANGED, getExpectedDetailObj(TABS_ID, 2, tabNumber))
					.get('@tabs')
					.invoke('attr', ATTRS.SELECTED_TAB, tabNumber);
				checkTabSelected(tabNumber);
	...
	context(`Tabs controlled using custom events`, () => {
		...
		it(`Should respond correctly when SET_PREV_TAB and SET_NEXT_TAB custom events dispatched`, () => {
			cy.addCustomEventListener(EVENTS.OUT.SELECTED_TAB_CHANGED, getExpectedDetailObj(TABS_ID, 1, 2))
